...
import Keyring from './keyring1'
import { IS_SESSION_STORAGE_SUPPORTED } from './keyring1/VaultStorage'
import { openInNewTab } from '@shared/utils'
import { MSG_CONNECT } from '@core/content-script/keep-bg-alive'
...
if (!IS_SESSION_STORAGE_SUPPORTED) {
  Keyring.on('lockedStatusUpdate', async (isLocked: boolean) => {
    if (!isLocked) {
