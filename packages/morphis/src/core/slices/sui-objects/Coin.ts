...
export class Coin {
  ...
  private static async selectGasPayment(
    ...
    const gasPayment = await CoinAPI.selectCoinWithBalanceGreaterThanOrEqual(
      coins,
      amount,
      exclude
    )
  ...
  public static async transferSui(
    ...
    const targetAmount = amount + BigInt(DEFAULT_GAS_BUDGET_FOR_TRANSFER_SUI)
  ...
  private static async requestSuiCoinWithExactAmount(
    ...
    await Coin.transferSui(signer, coins, amount, await signer.getAddress())
